...
export default function Dashboard() {
    ...
    useEffect(() => {
      ...
      if (!token && !authState.isLoading && !authState.loggedIn) {
        router.replace("/login");
        return;
      }  
      if (token && !authState.isTokenThere) {
        dispatch(setTokenIsThere());
      }

      }, [authState.isLoading, authState.isTokenThere, authState.loggedIn]);
                      ...
                      return(
                        <div className={styles.singleComment} key={postComment._id}>
                          <div className={styles.singleComment__profileContainer}>
                            <img scr={`${BASE_URL}/${postComment.userId?.profilePicture}`} alt=''/>
                            <div>
